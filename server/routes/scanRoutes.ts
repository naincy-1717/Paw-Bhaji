...
router.post('/decode-image', async (req, res) => {
  try {
    ...
    for (const modelName of candidateModels) {
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          timer = setTimeout(() => reject(new Error('AI vision timed out')), 8000);
        });
        ...
        // Model temporarily busy; proceed to next model
      }
    }
    ...
    res.status(200).json({ success: false, message: 'AI vision scanner temporarily busy. Please use laser scanner or manual entry.' });
  }
});
