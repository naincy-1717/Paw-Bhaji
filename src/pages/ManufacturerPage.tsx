...
interface ManufacturerPageProps {
  ...
  activeSubTab?: 'overview' | 'dispatch' | 'chat' | 'my-products';
  onTabChange?: (tab: 'overview' | 'dispatch' | 'chat' | 'my-products') => void;
}
...
export const ManufacturerPage: React.FC<ManufacturerPageProps> = ({
  ...
  // Navigation tabs - synchronized with App and Sidebar
  const [internalTab, setInternalTab] = useState<'overview' | 'dispatch' | 'chat' | 'my-products'>('overview');
  const activeTab = activeSubTab || internalTab;
  const setActiveTab = (tab: 'overview' | 'dispatch' | 'chat' | 'my-products') => {
    setInternalTab(tab);
    onTabChange?.(tab);
  };
    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-3">
        ...
        >
          ...
          Dispatch Stock as per Vacancy
          <span className="ml-1 rounded-full bg-amber-500/30 px-1.5 py-0.2 text-[10px] font-bold">
            Send
          </span>
        </button>
        ...
        <button
          ...
          className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all ${
            activeTab === 'my-products'
              ? 'bg-purple-100 text-purple-950 border border-purple-400 font-extrabold shadow-xs'
              : 'text-slate-700 hover:text-slate-950 hover:bg-slate-100'
          }`}
        >
          <Boxes className="h-3.5 w-3.5 text-purple-800" />
          My Factory Stock (Manual List)
          <span className="ml-1 rounded-full bg-purple-200 px-1.5 py-0.2 text-[10px] font-mono font-black text-purple-950 border border-purple-300">
      ...
      {/* TAB 2: DISPATCH STOCK AS PER VACANCY */}
      {activeTab === 'dispatch' && (
        <div className="space-y-6">
          <div className="rounded-2xl border border-amber-300 bg-amber-50/70 p-4 text-xs text-amber-950 flex flex-col sm:flex-row sm:items-center justify-between gap-3 font-medium">
            <div className="flex items-center gap-2">
              <Truck className="h-5 w-5 text-amber-700 shrink-0" />
              <div>
                <strong className="font-black text-amber-950 block text-sm">Direct Stock Dispatch Console:</strong>
                <span>Select a warehouse with open storage vacancy, allocate units from your factory stock, and generate an inbound dispatch consignment.</span>
              </div>
            </div>
            <button
              type="button"
              onClick={() => setActiveTab('overview')}
              className="shrink-0 rounded-lg border border-amber-400 bg-white px-3 py-1.5 text-xs font-bold text-amber-950 hover:bg-amber-100 transition-colors"
            >
              View Full Vacancy Matrix →
            </button>
          </div>
            ...
            <div className="lg:col-span-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
              <div className="border-b border-slate-100 pb-3">
                <h3 className="text-base font-black text-slate-900 flex items-center gap-2">
                  ...
                  Dispatch Consignment Form
                </h3>
                <p className="text-xs text-slate-600 font-medium mt-0.5">
                  Goods are dispatched based on verified open bin capacities.
                </p>
              </div>

              <form onSubmit={handleDispatchStock} className="space-y-3.5">
                {/* Warehouse Selector */}
                <div>
                  <label className="text-xs font-black text-slate-900 block mb-1">
                    Select Destination Warehouse (Live Vacancy)
                  </label>
                  <select
                    required
                    value={selectedWarehouseId}
                    onChange={(e) => setSelectedWarehouseId(e.target.value)}
                    className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-950 font-bold focus:border-amber-600 focus:outline-hidden"
                  >
                    {warehouses.map((wh) => (
                      <option key={wh.warehouseId} value={wh.warehouseId}>
                        {wh.code}: {wh.name} — ({wh.availableVacancy} units free space / {wh.vacancyPercentage}% vacancy)
                      </option>
                    ))}
                  </select>
                  {selectedWarehouse && (
                    <div className="mt-1.5 p-2 rounded-lg bg-slate-50 border border-slate-200 text-xs flex items-center justify-between">
                      ...
                      <span className="font-extrabold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded text-[11px]">
                        {selectedWarehouse.availableVacancy} units available
                      </span>
                    </div>
                  )}
                </div>

                {/* Factory Stock Quick-Pick */}
                <div>
                  <label className="text-xs font-black text-slate-900 block mb-1">
                    Select Product from Your Factory Stock
                  </label>
                  <select
                  ...
                  >
                    ...
                    {myProducts.map((p) => (
                      <option key={p.sku} value={p.sku}>
                        {p.name} (SKU: {p.sku}) — {p.unitsPresent} units available at factory
                      </option>
                    ))}
                  </select>
                  {selectedProduct && (
                    <div className="mt-1 text-[11px] text-amber-900 font-bold flex items-center justify-between">
                      <span>Available at your factory: <strong>{selectedProduct.unitsPresent} units</strong></span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-black text-slate-900 block mb-1">Product Name</label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-950 font-semibold focus:border-amber-600 focus:outline-hidden"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-black text-slate-900 block mb-1">SKU Code</label>
                    <input
                      type="text"
                      required
                      placeholder="e.g. APX-SEN-01"
                      value={sku}
                      onChange={(e) => setSku(e.target.value)}
                      className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-950 font-mono uppercase font-bold focus:border-amber-600 focus:outline-hidden"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-black text-slate-900 block mb-1">Barcode (1D / 2D)</label>
                    <input
                      type="text"
                      placeholder="e.g. 8901001001"
                      value={barcode}
                      onChange={(e) => setBarcode(e.target.value)}
                      className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-950 font-mono font-bold focus:border-amber-600 focus:outline-hidden"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-black text-slate-900 block mb-1">Category</label>
                    <select
                      value={category}
                ...
                {/* Units & Expected Arrival */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-xs font-black text-slate-900">Units to Dispatch</label>
                      {selectedProduct && (
                        <span className="text-[10px] text-amber-900 font-bold">
                          Max: {selectedProduct.unitsPresent}
                        </span>
                      )}
                    </div>
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-950 font-mono font-black focus:border-amber-600 focus:outline-hidden"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-black text-slate-900 block mb-1">Expected Delivery Date</label>
                    <input
                      type="date"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-black text-slate-900 block mb-1">Carrier / Truck Fleet</label>
                    <input
                      type="text"
                      value={carrierName}
                  ...
                  <div>
                    <label className="text-xs font-black text-slate-900 block mb-1">Dispatch Notes</label>
                    <input
                      type="text"
                      placeholder="e.g. Fragile batch, handle with pallet jack"
                      value={notes}
                ...
                <div className="pt-2">
                  <button
                    type="submit"
                    disabled={submitting}
                    className="w-full py-3 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-black transition-all shadow-md shadow-amber-900/20 disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
                  >
                    <Truck className="h-4 w-4" />
                    <span>{submitting ? 'Confirming Dispatch...' : 'Confirm & Dispatch Stock as per Vacancy'}</span>
                  </button>
                </div>
              </form>
            </div>

            {/* Right Column: Live Vacancy Snapshot for Quick Routing */}
            <div className="lg:col-span-5 space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-3">
                <h4 className="text-xs font-black uppercase tracking-wider text-slate-500">
                  Target Fulfillment Hubs
                </h4>
                <div className="space-y-2.5">
                  {warehouses.map((wh) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="text-xs font-black text-slate-950">{wh.name}</div>
                            <div className="text-[11px] text-slate-600">{wh.city} • {wh.address}</div>
                          </div>
                          <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
                            wh.vacancyPercentage > 40
                              ? 'bg-emerald-100 text-emerald-900 border border-emerald-300'
                              : wh.vacancyPercentage > 15
                              ? 'bg-amber-100 text-amber-900 border border-amber-300'
                              : 'bg-rose-100 text-rose-900 border border-rose-300'
                          }`}>
                            {wh.vacancyPercentage}% Free
                          </span>
                        </div>
                        <div className="mt-2 text-[11px] text-slate-700 flex items-center justify-between font-medium">
                          <span>Free Bins: <strong>{wh.availableVacancy} units</strong></span>
                          <span>Total: {wh.totalCapacity}</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              {/* Inbound Tracking summary */}
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-black text-slate-900">Inbound Shipments in Transit</span>
                  <span className="text-[10px] font-bold text-slate-500">{shipments.length} scheduled</span>
                </div>
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {shipments.slice(0, 4).map((s) => (
                    <div key={s.shipmentId} className="p-2 rounded-lg bg-slate-50 border border-slate-200 text-xs flex items-center justify-between">
                      <div>
                        <div className="font-bold text-slate-900">{s.productName}</div>
                        <div className="text-[10px] text-slate-500">{s.quantity} units • {s.carrierName}</div>
                      </div>
