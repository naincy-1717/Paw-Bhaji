...
export const DailyPickupScannerList: React.FC<DailyPickupScannerListProps> = ({
  ...
  return (
    <section className="space-y-4">
      ...
      {/* Live Camera Scanner Modal with Flip Horizontal, Macro Zoom & Retake Support */}
      <CameraScannerModal
        isOpen={isCameraActive}
        onClose={stopCamera}
        onScan={(code) => {
          handleConfirmPickup(code, selectedOrderForScan?.orderId);
        }}
        title={
          selectedOrderForScan
            ? `Scan: ${selectedOrderForScan.productName}`
            : 'Scan Delivery Pickup Barcode'
        }
        expectedBarcode={selectedOrderForScan?.barcode}
        expectedProductName={
          selectedOrderForScan
            ? `${selectedOrderForScan.productName} (${selectedOrderForScan.orderId})`
            : undefined
        }
                  ...
                  {isConfirmed ? (
                    ...
                    <div className="flex items-center gap-2 w-full">
                      <button
                        type="button"
                        onClick={() => onLocateInsideWarehouse?.(item)}
                        className="flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-slate-100 hover:bg-slate-200 px-3 py-2 text-xs font-bold text-slate-800 transition-all shrink-0 cursor-pointer"
                      ...
                      {/* Item-specific Camera Scan Button */}
                      <button
                        type="button"
                        onClick={() => {
                          setSelectedOrderForScan(item);
                          startCamera();
                        }}
                        className="flex items-center justify-center gap-1.5 rounded-xl border border-emerald-600/30 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 px-3 py-2 text-xs font-bold transition-all shrink-0 cursor-pointer"
                        title="Scan package barcode with camera"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 px-3.5 py-2 text-xs font-black text-white transition-all shadow-xs cursor-pointer disabled:opacity-50"
                      >
                        <BarcodeIcon className="h-4 w-4 shrink-0" />
                        <span className="truncate">Confirm Pickup (+{item.quantity})</span>
                      </button>
                    </div>
                  )}
                </div>
