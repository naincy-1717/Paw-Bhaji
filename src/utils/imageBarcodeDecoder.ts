...
export async function decodeBarcodeFromImageFile(file: File): Promise<BarcodeDecodeResult> {
  ...
  if (typeof window !== 'undefined' && 'BarcodeDetector' in window) {
    try {
      ...
      if (ctx) {
        ...
        // Test horizontally flipped (critical for webcam mirror / selfie photos)
        const flipCanvas = document.createElement('canvas');
        flipCanvas.width = targetW;
        flipCanvas.height = targetH;
        const flipCtx = flipCanvas.getContext('2d');
        if (flipCtx) {
          flipCtx.translate(targetW, 0);
          flipCtx.scale(-1, 1);
          flipCtx.drawImage(canvas, 0, 0);
          const flipResults = await detector.detect(flipCanvas);
          if (flipResults && flipResults.length > 0) {
            const flipList = flipResults
              .map((r: any) => ({ value: r.rawValue?.trim(), format: r.format }))
              .filter((r: any) => Boolean(r.value));

            if (flipList.length > 0) {
              const classified = classifyCodes(flipList);
              return {
                success: true,
                barcode: classified.barcode,
                serialNumber: classified.serialNumber || undefined,
                allCodes: classified.allCodes,
                format: flipResults[0].format || 'Native Mirrored',
                dataUrl,
                fileName,
                fileSize,
              };
  ...
  try {
    ...
    html5QrCode = new Html5Qrcode(tempContainerId, {
      formatsToSupport,
      verbose: false,
    });
    ...
    try {
      ...
      if (ctx) {
        ...
        if (flipCtx) {
          ...
          flipCtx.scale(-1, 1);
          flipCtx.drawImage(canvas, 0, 0);
          const flippedFile = await canvasToFile(flipCanvas, 'flipped.jpg', 'image/jpeg', 0.95);
          const decodedFlipped = await html5QrCode.scanFile(flippedFile, false);
          if (decodedFlipped && decodedFlipped.trim()) {
            return {
              success: true,
              barcode: decodedFlipped.trim(),
              format: 'Mirrored Barcode',
              dataUrl,
              fileName,
              fileSize,
            };
